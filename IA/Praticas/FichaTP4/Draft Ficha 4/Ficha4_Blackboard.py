...
#Função responsável por aplicar o algoritmo minimax para determinar o melhor movimento para o computador
#Retorna uma lista com a melhor jogada para o computador
# Algoritmo MINIMAX: https://en.wikipedia.org/wiki/Minimax
#RACIOCINIO DO ALGORITMO:
#Se o jogador for o computador, o algoritmo maximiza o valor da jogada
#Se o jogador for o humano, o algoritmo minimiza o valor da jogada
#O algoritmo é recursivo, e a cada chamada, a profundidade é decrementada
#O algoritmo termina quando a profundidade é 0 ou se o jogo tiver terminado
#O algoritmo avalia o estado do jogo e retorna um valor
#O algoritmo retorna a melhor jogada para o computador
def minimax(estado, profundidade, jogador):
    #se o jogador for o computador (BOT), o melhor movimento é o menor valor possivel
    if jogador == BOT:
        melhor = [-1, -1, -infinito]
    else:
        melhor = [-1, -1, infinito]

    #se a profundidade for 0 ou se o jogo tiver terminado retorna o valor da avaliação do estado atual
    ...
    for celula in celulas_vazias(estado):
        ...
        estado[x][y] = jogador
        #se o jogador for o computador
        if jogador == BOT:
            pontuacao = minimax(estado, profundidade - 1, HUMANO) #pontuação toma o valor da avaliação do estado atual, ou seja, a melhor jogada para o computador
            pontuacao[0], pontuacao[1] = x, y
            if pontuacao[2] > melhor[2]:
                melhor = pontuacao
        else:
            pontuacao = minimax(estado, profundidade - 1, BOT) #pontuação toma o valor da avaliação do estado atual, ou seja, a melhor jogada para o humano
            pontuacao[0], pontuacao[1] = x, y
            if pontuacao[2] < melhor[2]: #se a pontuação for menor que a melhor pontuação, a melhor pontuação toma o valor da pontuação
                melhor = pontuacao

        estado[x][y] = 0 #atualiza o estado do tabuleiro para 0
